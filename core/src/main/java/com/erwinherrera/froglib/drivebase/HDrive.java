package com.erwinherrera.froglib.drivebase;
...
public class HDrive extends RobotDrive {
    ...
    public void driveFieldCentric(double strafeSpeed, double forwardSpeed, double turn, double heading) {
        ...
        if (speeds.length == 3) {
            ...
            motors[MotorType.kLeft.value].swim(speeds[MotorType.kRight.value] * maxOutput);
            motors[MotorType.kRight.value].swim(speeds[MotorType.kLeft.value] * maxOutput);
            motors[MotorType.kSlide.value].swim(speeds[MotorType.kSlide.value] * maxOutput);
        }
        // this looks similar to mecanum because mecanum is a four wheel holonomic drivebase
            ...
            motors[MotorType.kFrontLeft.value]
                    .swim(speeds[MotorType.kFrontLeft.value] * maxOutput);
            motors[MotorType.kFrontRight.value]
                    .swim(speeds[MotorType.kFrontRight.value] * -maxOutput);
            motors[MotorType.kBackLeft.value]
                    .swim(speeds[MotorType.kBackLeft.value] * maxOutput);
            motors[MotorType.kBackRight.value]
                    .swim(speeds[MotorType.kBackRight.value] * -maxOutput);
        }
