package com.erwinherrera.froglib.behaviors;

import com.erwinherrera.froglib.drivebase.MecanumDrive;
import com.erwinherrera.froglib.geometry.Pose2d;
import com.erwinherrera.froglib.purepursuit.Path;
import com.erwinherrera.froglib.purepursuit.Waypoint;

/**
 * @author Erwin Herrera
 * @see Path
 */
public class PurePursuitBehavior extends BehaviorBase {

    private MecanumDrive m_drive;
    private OdometryNest m_odometry;
    private Path m_path;

    public PurePursuitBehavior(MecanumDrive drive, OdometryNest odometry, Waypoint... waypoints) {
        m_path = new Path(waypoints);
        m_drive = drive;
        m_odometry = odometry;
    }

    @Override
    public void onDive() {
        m_path.init();
    }

    public void addWaypoint(Waypoint waypoint) {
        m_path.add(waypoint);
    }

    public void addWaypoints(Waypoint... waypoints) {
        for (Waypoint waypoint : waypoints) this.addWaypoint(waypoint);
    }

    public void removeWaypointAtIndex(int index) {
        m_path.remove(index);
    }

    /**
     * Call this in a loop
     */
    @Override
    public void onPaddle() {
        Pose2d robotPose = m_odometry.getPose();
        double[] motorSpeeds = m_path.loop(robotPose.getTranslation().getX(), robotPose.getTranslation().getY(), robotPose.getHeading());
        m_drive.driveRobotCentric(motorSpeeds[0], motorSpeeds[1], motorSpeeds[2]);
    }

    @Override
    public void onFly(boolean interrupted) {
        m_drive.stop();
    }

    @Override
    public boolean hasSurfaced() {
        return m_path.isFinished();
    }

}
