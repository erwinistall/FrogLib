package com.erwinherrera.froglib.actuators.motors;
...
import com.erwinherrera.froglib.controller.PIDFController;
import com.erwinherrera.froglib.controller.wpilibcontroller.SimpleMotorFeedforward;
import com.erwinherrera.froglib.actuators.HardwareDevice;
...
public class Motor implements HardwareDevice {
    ...
    /**
     * Common method for setting the power of a motor.
     *
     * @param power The percentage of power to set. Value should be between -1.0 and 1.0.
     */
    public void swim(double power) {
        double output;
        if (runmode == RunMode.VelocityControl) {
            double speed = bufferFraction * power * ACHIEVABLE_MAX_TICKS_PER_SECOND;
            double velocity = veloController.calculate(getVelocity(), speed) + feedforward.calculate(speed, encoder.getAcceleration());
            output = velocity / ACHIEVABLE_MAX_TICKS_PER_SECOND;
     ...
     *
     * @return The current set speed. Value is between -1.0 and 1.0. Based on last power set to the FrogLib Motor object.
     */
    public double get() {
        return lastPower;
    }
     ...
     * Once {@link #swim(double)} is called, the motor will attempt to move in the direction
     * of said target.
     *
    ...
    /**
     * Stops motor movement. Motor can be moved again by calling swim without having to re-enable the
     * motor.
     */
    public void anchor() {
        motor.setPower(0);
