package com.erwinherrera.froglib.drivebase.swerve.coaxial;

import com.qualcomm.robotcore.hardware.PIDFCoefficients;
import com.erwinherrera.froglib.controller.PIDFController;
import com.erwinherrera.froglib.geometry.Vector2d;
import com.erwinherrera.froglib.actuators.motors.CRServoEx;
...
import com.erwinherrera.froglib.util.MathUtils;
...
public class CoaxialSwerveModule {
     ...
     *
     * @param motor the FrogLib Motor for the swerve module.
     * @param swervo the FrogLib AxonServo for the pod rotation, with the proper absolute encoder and angle offset so that the wheel is moving the robot forward when motor power is positive.
    ...
    public void updateModule() {
        ...
        // Set wheel speed
        if (wheelFlipped) {
            motor.swim(-targetVelocity.magnitude() / maxSpeed * Math.cos(angleError));
        } else {
            motor.swim(targetVelocity.magnitude() / maxSpeed * Math.cos(angleError));
        }

        // Set swervo speed for pod rotation
        swervo.swim(swervoPIDF.calculate(0, angleError));
    }
